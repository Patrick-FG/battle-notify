...
module.exports = function EntityManager(dispatch, debug){
    ...
    dispatch.hook('S_PARTY_MEMBER_LIST', (dispatch.base.majorPatchVersion >= 67) ? 6 : 5, processPartyList)
    //dispatch.hook('S_PARTY_MEMBER_INFO', 3, processPartyList)

    function processPartyList(event) {
        event.members.forEach(member => {
            const member_id = (dispatch.base.majorPatchVersion >= 67) ? member.gameId : member.cid
            const entity = getEntity(member_id)
